...
function refresh() {
  ...
  else { renderOrderList(); renderCart(); renderHistory(); }
}
...
function renderOrderList() {
  ...
  if (!rows.length) { list.innerHTML = '<p class="empty">Ничего не найдено</p>'; return; }
  var max = 300;
...
function renderCart() {
  ...
  if (!ids.length) {
    box.innerHTML = '<p class="empty" style="padding:1rem 0">Корзина пуста</p>';
...
function saveCurrentOrder() {
  ...
  var list = loadOrders(); list.unshift(order); saveOrders(list); renderHistory();
...
function bind() {
  ...
  $$(".tab").forEach(function (tab) {
    tab.addEventListener("click", function () {
      ...
      var mf = $("#matrix-filters"); if (mf) mf.hidden = currentView === "order";
      refresh();
    });
  });
  document.addEventListener("click", function (e) {
    ...
    if (del) { if (confirm("Удалить заказ из истории?")) deleteOrder(del); }
  });
