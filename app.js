...
function refresh() {
  ...
  else if (currentView === "order") { renderOrderList(); renderCart(); renderHistory(); if (window.renderBudget) renderBudget(); }
  else if (currentView === "other") { if (window.renderOtherList) renderOtherList(); if (window.renderBudget) renderBudget(); }
}
...
function saveCurrentOrder() {
  ...
  var left = MONTHLY_BUDGET - (window.ordersSpentThisMonth ? ordersSpentThisMonth() : 0) - (window.otherSpentThisMonth ? otherSpentThisMonth() : 0);
  alert("Заказ сохранён · " + fmt(order.total) + "\nОстаток бюджета: " + fmt(left));
}
...
function bind() {
  ...
  document.addEventListener("click", function (e) {
    ...
    if (del) { if (confirm("Удалить заказ из истории?")) { deleteOrder(del); if (window.renderBudget) renderBudget(); } return; }
    var delO = t.getAttribute("data-del-other");
    if (delO && window.deleteOther) { if (confirm("Удалить расход?")) deleteOther(delO); }
  });
  ...
  if (clearBtn) clearBtn.addEventListener("click", function () { CART = {}; saveCart(); renderCart(); if (window.renderBudget) renderBudget(); refresh(); });
  if (window.initOtherUI) initOtherUI();
}
